...
class OP():
    ...
    the bit pattern.  Treat this class as a final maybe and
    get other data for position and resulting type
     * registers
     * literal
    ...
    def val(self, fmt, index, total):
        return "oper%x%x%x_%x%x%x%x" % (fmt, index, total, self.align, self.lit, self.fp, self.sfr)
...
#      for the operands
def M1(opc):
    pass # opc.bitpattern["m1"]=1
def M2(opc):
    pass # opc.bitpattern["m2"]=1
def M3(opc):
    pass # opc.bitpattern["m3"]=1
def S1(opc):
    pass # opc.bitpattern["s1"]=1
def S2(opc):
    pass # opc.bitpattern["s2"]=1
...
def do_global(tbl):
    """print the tbl"""
    if tbl in ["m1", "m2", "m3", "s1", "s2", "disp0212", "disp0223", "op2431", "op0710", "op0000"]:
        return ""
        ...
        #TODO  not sure, but make REG src/dst and MEM is dst... this might be true for all
        #      either way scraping this data is broken
        if (fmt == REG or fmt in [MEM1,MEM2,MEM4,MEM8,MEM12,MEM16]) and total == 2 and lit ==0 and index == 1:
            index = 2
...
class Opcode():
    ...
    def __str__(self):
        ...
        for x in range(len(self.operands)):
            _ = self.operands[x]
            if isinstance(_, OP):
                tmp2.append(_.val(self.fmt, x, len(self.operands)))
                tmp.append(_.val(self.fmt, x, len(self.operands)))
            else:
                if _.startswith("efa"):
                    efa = _
                else:
    ...
    def parse(self):
        ...
        for x in range(len(self.operands)):
            _ = self.operands[x]
            if isinstance(_, OP):
                global_tables.append(_.val(self.fmt, x, len(self.operands)))
            else:
...
opcodes = [
	...
	( REG_OPC, 0x800, "ldob",		I_BASE,	MEM1,	2, ( M,  R,  0 ) ),
	( REG_OPC, 0x820, "stob",		I_BASE,	MEM1,	2, ( R,  M,  0 ) ),
	( REG_OPC, 0x840, "bx",		I_BASE,	MEM1,	1, ( M,  0,  0 ) ),
	( REG_OPC, 0x850, "balx",		I_BASE,	MEM1,	2, ( M,  R,  0 ) ),
	( REG_OPC, 0x860, "callx",	I_BASE,	MEM1,	1, ( M,  0,  0 ) ),
	( REG_OPC, 0x880, "ldos",		I_BASE,	MEM2,	2, ( M,  R,  0 ) ),
	( REG_OPC, 0x8a0, "stos",		I_BASE,	MEM2,	2, ( R,  M,  0 ) ),
	( REG_OPC, 0x8c0, "lda",		I_BASE,	MEM1,	2, ( M,  R,  0 ) ),
	( REG_OPC, 0x900, "ld",		I_BASE,	MEM4,	2, ( M,  R,  0 ) ),
	( REG_OPC, 0x920, "st",		I_BASE,	MEM4,	2, ( R,  M,  0 ) ),
	( REG_OPC, 0x980, "ldl",		I_BASE,	MEM8,	2, ( M,  R2, 0 ) ),
	( REG_OPC, 0x9a0, "stl",		I_BASE,	MEM8,	2, ( R2, M,  0 ) ),
	( REG_OPC, 0xa00, "ldt",		I_BASE,	MEM12,	2, ( M,  R4, 0 ) ),
	( REG_OPC, 0xa20, "stt",		I_BASE,	MEM12,	2, ( R4, M,  0 ) ),
	( REG_OPC, 0xb00, "ldq",		I_BASE,	MEM16,	2, ( M,  R4, 0 ) ),
	( REG_OPC, 0xb20, "stq",		I_BASE,	MEM16,	2, ( R4, M,  0 ) ),
	( REG_OPC, 0xc00, "ldib",		I_BASE,	MEM1,	2, ( M,  R,  0 ) ),
	( REG_OPC, 0xc20, "stib",		I_BASE,	MEM1,	2, ( R,  M,  0 ) ),
	( REG_OPC, 0xc80, "ldis",		I_BASE,	MEM2,	2, ( M,  R,  0 ) ),
	( REG_OPC, 0xca0, "stis",		I_BASE,	MEM2,	2, ( R,  M,  0 ) ),
	...
	# /* Hx extensions.  */
	( REG_OPC, 0xac0, "dcinva",	I_HX,	MEM1,	1, (   M,  0, 0 ) ),
]
